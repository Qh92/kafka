...
轮询默认是将消费者组订阅的主题看成一个**整体**，将这个整体排序后轮询分配

这样的**好处**：消费者组中分区的差值最多差一个

看成一个整体的**坏处**：必须要求消费者组中的消费者订阅相同的topic，如果此时相同的消费者组中的消费者订阅不同的主题，可能造成消息发往没有订阅的消费者中去

**要求：必须要求消费者组中的消费者订阅相同的topic**



<img src="assets\image-20220104221010173.png" alt="image-20220104221010173" style="zoom: 33%;" />

<img src="assets\image-20220104220737022.png" alt="image-20220104220737022" style="zoom: 33%;" />
...
举例：当前topic 7个分区 7/3 = 2，第一个消费者消费三个分区，后面的两个消费两个分区

<img src="assets\image-20220104222347735.png" alt="image-20220104222347735" style="zoom:25%;" />

如果现在有三个topic，每个topic有四个分区，4 / 3 = 1，第一个消费者每个topic消费2个分区，其它的消费者消费1个分区，出现消费不对等问题



**消费者组中消费者个数发生增多或减少会触发重新分配**
